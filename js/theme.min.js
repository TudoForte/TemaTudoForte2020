...
if(document.querySelectorAll('.banner-home')[0] !== undefined ){
...
if(document.querySelectorAll('.product-slider')[0] !== undefined ){
...
if(document.querySelectorAll('.banner-brands')[0] !== undefined ){
...
if(document.querySelectorAll('.swiper-mobile-category')[0] !== undefined ){

  let swiperMobileCategory = new Swiper('.swiper-mobile-category', {
    slidesPerView: 3,
    spaceBetween: 10,
    slidesPerGroup: 3,
    loop: true,
...
// let swiperBannerRatingProduct = new Swiper ('#swiper-rating-product', {
//   slidesPerView: 1,
//     autoplay: {
//       delay: 3000,
//       disableOnInteraction: true,
...
// GET STAR PRODUCT
async function getRatingProductSetLocalStorage(){
...
getRatingProductSetLocalStorage() 
...
document.querySelectorAll ('[data-app]').forEach (async product => {
  ...
  if(localStorage.getItem("rating") !== null){
      ...
      let data = JSON.parse(localStorage.getItem("rating"));
        ...
        let ratingValue = data[productId].rate;
        let ratingCount = data[productId].nb_reviews;
          ...
          let elementRatingProductId = document.getElementById(`rating-${productId}`)
          
          elementRatingProductId.innerHTML = ratingComponent
...
async function getCart() {
  ...
  await axios
    ...
    .catch (function (error) {
        ...
        document.getElementById ('sidebar-cart').innerHTML = '<li><p>Carrinho Vazio!</p></li>';
        document.getElementById ('priceSubtotal').innerHTML = 0;
   
        document.getElementById ('wrapper-itens').innerHTML = '<div class="item><p>Carrinho Vazio!</p></div>';
...
function createLinkCart(){
  ...
  axios.get(URL_TO_FETCH_GET_CART).then(function (response){
      ...
      .then(data => {
        ...
        document.getElementById("sharedcart-input-whats").style.display = 'block';
        ...
        document.getElementById('sharedcart-buttons-whats').setAttribute('href', `http://api.whatsapp.com/send?text=${urlShareCart}`);
        ...
        setTimeout(function(){
          ...
          document.getElementById('sharedcart-input-whats').style.display = 'none';
...
function createLinkCartWithPhone(){
  ...
  let phone = document.getElementById('sharedcart-input-whats').value
  let urlButtonWhats = document.getElementById('sharedcart-buttons-whats').getAttribute('href')

  let urlShareCartWithPhone

  if(document.getElementById('sharedcart-input-whats').value == ''){
    urlShareCartWithPhone = `${urlButtonWhats}`;
  }else{
    urlShareCartWithPhone = `${urlButtonWhats}&phone=55${phone}`
  }
  
  document.getElementById('sharedcart-buttons-whats').setAttribute('href', `${urlShareCartWithPhone}`);
...
function jump(h){
    ...
    // if(h == 'payment'){
    //   document.getElementById('wrapper-payment').style.display = 'flex';
    // }
}
...
async function getCartHover() {
  let dataSession = document.getElementsByTagName ('html')[0].getAttribute('data-session');

  let URL = 'https://www.tudoforte.com.br';
  let URL_TO_FETCH_GET_CART = `${URL}/web_api/carts/${dataSession}/`;

  await axios.get(URL_TO_FETCH_GET_CART).then (function (response) {

      document.getElementById('wrapper-itens').innerHTML = '';
      ...
      let mapCart = response.data.map (item => {
        ...
        let itemCartHover = `
          <div class="item">
            ...
            <div class="item-description">
              <h4 class="item-description-title">${productName}</h4>
              <div class="item-wrapper-price">  
                <span class="items-quantity">Quantidade:${productQuantity}</span>
                <span class="item-price">R$ ${formatReal(productPriceFormat)}</span>
              </div>
            </div>
          </div>
        `
        document.getElementById('wrapper-itens').insertAdjacentHTML('beforeend', itemCartHover);
        ...
        // ADD PRICESUBTOTAL CARTHOVER
        let priceSubTotalAround = around10(priceSubTotal);
        let priceSubTotalString = priceSubTotalAround.toString();
        let priceSubTotalFormat = getMoney(priceSubTotalString);
  
        document.getElementById ('total').innerHTML = formatReal(priceSubTotalFormat);
...
// function search(query, limit=10, page=1) {

//   let URL_TO_FETCH_SEARCH = `/web_api/search?query=${query}&limit=${limit}&page=${page}`

//   axios({
//     method: 'get',
//     url: URL_TO_FETCH_SEARCH
//   }).then(response => {

//     console.log(response.data)

//     let arrayProduct = response.data.Products;

//     arrayProduct.forEach(item => {

//       if(item.Product.Category.id !== '1395'){
//         console.log(item.Product.Category.id)
//       }
...
// var intervIdLabelSend;

// function changeTextLabelSend() {
//     intervIdLabelSend = setInterval(flashTextLabelSend, 2000);
// }

// function flashTextLabelSend() {
//     var elemflashTextLabelSend = document.getElementById('labels-send').textContent;
    
//     if(elemflashTextLabelSend == 'Enviando'){
//         document.getElementById('labels-send').textContent = 'Normalmente'
//     }else{
//         document.getElementById('labels-send').textContent = 'Enviando'
//     }
// }
